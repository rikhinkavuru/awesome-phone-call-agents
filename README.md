...
| [`apps/typescript/shortline`](apps/typescript/shortline/) | TypeScript | Phone calls as a statistical sensor network for drug shortages: rotating stratified pharmacy panels dialled as CALL-E batch tasks, evidence-gated observations, a weekly availability index with Wilson intervals, and sourcing waves that reuse surveillance data; dry-run by default with a fake provider and fake HTTP server. |
