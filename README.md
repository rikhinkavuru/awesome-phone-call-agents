...
- [`shortline-find`](skills/shortline-find/) - Find pharmacies that can dispense a product today, or read a weekly street-level availability index, by planning and running Shortline sourcing waves over CALL-E; plans without calling and requires explicit approval before a wave.
...
| [`apps/typescript/shortline`](apps/typescript/shortline/) | TypeScript | Phone calls as a statistical sensor network for drug shortages: rotating stratified pharmacy panels dialled as CALL-E batch tasks, evidence-gated observations, a weekly availability index with Wilson intervals, and sourcing waves fed by fresh sightings; dry-run by default with a fake provider, a fake HTTP server, and a measured evaluation harness. |
...
- [`Evidence gates for phone observations`](apps/typescript/shortline/docs/call-design.md) - Why a schema-valid structured result is not yet an observation: the product must have been named, the quote must be in the callee's own turns, and everything else is recorded as nonresponse rather than as a no.
- [`Courtesy by sampling design`](apps/typescript/shortline/docs/statistics.md) - Rotating stratified panels with cooldowns make the courtesy budget a property of the sample instead of a rate limiter, and keep the resulting estimate honest with intervals, coverage, and response rate.
